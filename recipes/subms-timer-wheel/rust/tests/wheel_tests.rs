...
fn long_delay_spans_revolutions() {
    ...
    for _ in 0..(2 * n + 2) { assert!(w.tick().is_empty()); }
...
fn delay_zero_fires_on_next_tick_or_inline() {
    ...
    let mut fired = false;
    for _ in 0..64 {
        if !w.tick().is_empty() { fired = true; break; }
    }
    assert!(fired);
}
...
fn cancel_returns_false_on_already_fired() {
    ...
    for _ in 0..64 { if !w.tick().is_empty() { break; } }
...
fn many_pending_timers_fire_at_various_delays() {
    ...
    for i in 1..=50u32 { w.schedule(i as usize, i); }
    let mut total_fired = 0usize;
    for _ in 0..50 { total_fired += w.tick().len(); }
...
fn ticks_with_no_timers_return_empty() {
    ...
    for _ in 0..100 { assert!(w.tick().is_empty()); }
}
