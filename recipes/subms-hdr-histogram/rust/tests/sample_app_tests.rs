...
//! each optional feature (gated the same way as the sample).
...
fn coordinated_omission_lifts_the_tail() {
    ...
    assert!(corrected_p99 >= 500, "correction backfills the blocked requests: {corrected_p99}");
    assert!(corrected_p99 > naive_p99, "corrected tail is strictly higher");
}
...
fn decay_weights_recent_activity() {
    ...
    assert!(h.value_at_percentile(0.5) < 2_000, "recent fast ops dominate");
}
...
fn value_tagging_separates_venues() {
    ...
    assert!(h.value_at_percentile_for_tag(0.99, COLO) < h.value_at_percentile_for_tag(0.99, REMOTE));
}
