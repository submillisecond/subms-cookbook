...
fn handles_large_values() {
    ...
    assert!(h.max() as f64 >= big as f64 * 0.99, "max {} too low for {big}", h.max());
...
fn high_volume_record_stays_consistent() {
    let mut h = HdrHistogram::new(3);
    for i in 0..100_000u64 { h.record((i % 1000) + 1); }
