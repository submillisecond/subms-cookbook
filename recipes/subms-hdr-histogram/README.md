...
Part of the [submillisecond.com cookbook](https://www.submillisecond.com/cookbook/recipes/subms-hdr-histogram) - a `observability` recipe. Rust + Java, byte-equivalent JSON contract, zero runtime dependencies (`std` / JDK only; the perf harness is an opt-in Cargo feature).

## Install

```toml
# Cargo.toml
subms-hdr-histogram = "0.10"
```
...
<dependency>
  ...
  <artifactId>subms-hdr-histogram</artifactId>
...
- [`rust/`](./rust/) - Rust edition 2024, `std`-only library (crate `subms_hdr_histogram`).
