...
/**
 * Hand-rolled {@link MetricExporter} that prints every flushed metric to a
 * {@link PrintStream}. Replaces the heavier {@code opentelemetry-exporter-logging-otlp}
 * artefact so the bridge stays dependency-light.
 *
 * <p>One block per metric: name, unit, then a per-point attribute set + count
 * + sum. Adequate for the autoconfig "look, OTEL data came out" fallback; not
 * a substitute for OTLP in production.
 */
