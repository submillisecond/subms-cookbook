...
fn sub_millisecond_bench() {
    let params = SubMsBenchParams { entries: 50_000, warmup: 1_000, seed: 0 };
    let h = run_bench(&HyperLogLogRecipe, &params);
    assert_p99_under(
        &h,
        &[
            SubMsBenchAssertion { stage: "add",      p99_ns_max: ONE_MS_NS },
            SubMsBenchAssertion { stage: "estimate", p99_ns_max: ONE_MS_NS },
        ],
    ).unwrap();
}
