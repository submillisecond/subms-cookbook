...
Part of the [submillisecond.com cookbook](https://www.submillisecond.com/cookbook/recipes/subms-hyperloglog) - a `probabilistic` recipe. Rust + Java, byte-equivalent JSON contract, zero runtime dependencies (`std` / JDK only; the perf harness is an opt-in Cargo feature).

## Install

```toml
# Cargo.toml
subms-hyperloglog = "0.10"
```
...
- [`rust/`](./rust/) - Rust edition 2024, `std`-only library (crate `subms_hyperloglog`).
