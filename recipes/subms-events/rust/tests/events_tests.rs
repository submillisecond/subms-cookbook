...
use subms_events::{
    CompositeListener, Event, EventDispatcher, EventLevel, FilterListener, OverflowPolicy, listener,
};
...
fn bridge_receives_events() {
    use subms_events::{EventBridge, EventDispatcher};
...
type GatedListener = (
    Arc<dyn subms_events::EventListener>,
...
fn bridge_listener_exposes_name_and_flush_is_noop() {
    use subms_events::{BridgeListener, EventBridge};
