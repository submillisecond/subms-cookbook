...
fn open_order_set_scenario() {
    ...
    for oid in ["ORD-1001", "ORD-1003", "ORD-1004"] {
        assert!(open.contains(oid), "a stored order must always report present");
    }
...
fn variable_fingerprint_lowers_false_positives() {
    ...
    assert!(wide_fp < narrow_fp, "wide_fp={wide_fp} should be < narrow_fp={narrow_fp}");
}
...
fn snapshot_is_frozen_against_later_writes() {
    ...
    let matched = (0..1_000u32).filter(|i| snap.contains(&format!("ORD-{i}"))).count();
    assert_eq!(matched, 1_000, "the snapshot keeps the whole captured set");
    assert!(snap.contains("ORD-0"), "snapshot retains a key the writer later deleted");
    assert!(!snap.contains("ORD-LATE"), "snapshot never sees the writer's later insert");
}
...
fn compressed_footprint_beats_base_at_moderate_load() {
    ...
    assert!(cf.occupied_bytes() < base_fixed_bytes, "sorted-run encoding is smaller here");
    for i in 0..3_000u32 {
