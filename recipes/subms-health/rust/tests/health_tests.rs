...
use subms_health::{
    Clock, ComponentHealth, EnvSection, FixedClock, HealthConfig, HealthRegistry, HealthStatus,
    MapEnv, ProbeKind, RedactionPolicy, RefreshPolicy, http_status_for,
};
...
fn status_change_callback_fires_non_blocking() {
    use std::sync::Mutex;
    use subms_health::{Event, on_status_change};
...
fn sync_dispatch_has_no_threads() {
    use std::sync::Mutex;
    use subms_health::on_status_change;
