#!/usr/bin/env node
// Standardised cookbook bench runner + verifier.
//
// Walks `recipes/`, runs every recipe's rust + java perf bench, captures
// the JSON output to `<recipe>/perf/<lang>.json`, then verifies that
// every captured stage holds the sub-millisecond p99 claim.
//
// Usage:
//   node scripts/bench-all.mjs                  rust + java, every recipe
//   node scripts/bench-all.mjs --rust           rust only
//   node scripts/bench-all.mjs --java           java only
//   node scripts/bench-all.mjs --check          verify existing JSONs, no run
//   node scripts/bench-all.mjs --only=foo,bar   filter to specific slugs
//   node scripts/bench-all.mjs --report-only    don't run, only print table
//   node scripts/bench-all.mjs --parallel=N     up to N recipes at once (default 1)
//   node scripts/bench-all.mjs --clean          delete existing perf JSONs first
//   node scripts/bench-all.mjs --threshold=NS   override the 1ms (1_000_000ns) bar
//   node scripts/bench-all.mjs --md=PATH        write a markdown report
//   node scripts/bench-all.mjs --json=PATH      write a machine-readable summary
//   node scripts/bench-all.mjs --skip-warmup=N  drop the first N downsampled
//                                               samples (default 50 = 10%) so
//                                               JIT-cold / cache-cold passes
//                                               don't pollute the percentile.
//   node scripts/bench-all.mjs --top=N          rows in best/worst tables (default 8)
//
// Exit codes:
...
console.log();
console.log(`${c.bold}cookbook bench-all${c.reset}`);
...
async function benchRust(slug) {
  ...
  const dest = join(RECIPES_ROOT, slug, "perf", "rust.json");
...
async function benchJava(slug) {
  ...
  const json = extractJsonBlob(out);
  if (!json) return { status: "failed", reason: "no JSON in stdout", elapsed };
  const dest = join(RECIPES_ROOT, slug, "perf", "java.json");
  await mkdir(dirname(dest), { recursive: true });
  await writeFile(dest, json + "\n");
  return { status: "ok", elapsed, path: dest };
}
...
// ---------------- clean ----------------
if (cleanFirst) {
  console.log(`${c.yellow}--clean: removing existing perf JSONs${c.reset}`);
  for (const slug of slugs) {
    for (const lang of ["rust", "java"]) {
      const p = join(RECIPES_ROOT, slug, "perf", `${lang}.json`);
...
for (const slug of slugs) {
  ...
  for (const lang of ["rust", "java"]) {
    const path = join(RECIPES_ROOT, slug, "perf", `${lang}.json`);
