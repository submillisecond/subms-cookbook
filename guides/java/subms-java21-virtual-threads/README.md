...
- `src/main/java/com/submillisecond/guides/java21/VirtualThreadDemo.java`
  the smallest possible "is this thing virtual" - spawn 100k, each parks,
  assert they all complete.
- `src/main/java/com/submillisecond/guides/java21/ConcurrencyBench.java`
  head-to-head against a platform pool on a fan-out workload.
- `src/main/java/com/submillisecond/guides/java21/PatternMatchingDemo.java`
  sealed `OrderEvent` ADT, switch with record patterns and guards,
  `reversed()` and `firstEntry()` on sequenced collections.
- `src/test/java/com/submillisecond/guides/java21/VirtualThreadDemoTest.java`
  JUnit 5 checks for the virtual-thread API.
- `src/test/java/com/submillisecond/guides/java21/PatternMatchingDemoTest.java`
