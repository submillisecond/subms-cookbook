...
type: guide
category: java
repoPath: guides/java/subms-spring-boot-virtual-threads
order: 22
difficulty: 2
loc: 380
languages: [java]
prereqs:
  ...
  - "Java virtual threads (see the Java 21 guide)"
...
This guide is a minimum-viable demonstration: one fan-out endpoint, an in-process HTTP load driver, and a measured comparison of what that property changes about the tail.
...
```sh
cd cookbook/guides/java/subms-spring-boot-virtual-threads
...
java -cp "target/classes;$(cat target/cp.txt)" \
     com.submillisecond.guides.springboot.PerfMain bench-params.txt > perf.json
